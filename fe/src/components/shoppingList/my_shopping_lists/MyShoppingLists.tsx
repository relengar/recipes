...
function MyShoppingLists() {
  ...
  const { data, loading } = useQuery(GET_MY_SHOPPING_LISTS, {
    variables: { userId },
  });
            ...
            <div key={list.id} className="columns is-centered">
              <NavLink to={`/shopping-list/${list.id}`}>
                <div className="box column recipe is-full">
                  <div className="subtitle has-text-centered has-text-dark is-size-4">
                    {list.recipeTitle}
                  </div>
                  <div className="has-text-centered">
                    <ul>
                      {list.ingredients.map(
                        ({ amount, unit, original }: any, i: number) => (
                          <li
                            className="columns ingredient has-text-weight-meduim"
                            key={i}
                          >
                            <span className="column has-text-left is-three-fifths">
                              {original}
                            </span>
                            <i className="column is-two-fifths has-text-right">
                              {amount} {unit}
                            </i>
                          </li>
                        )
