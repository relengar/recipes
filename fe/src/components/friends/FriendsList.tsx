...
function notInFriends(user: UserInterface, friends: any): boolean {
  ...
  return !friends.myConnections.find(
...
function FriendsList() {
  ...
  const { data: friends, client } = useQuery<{
    myConnections: UserInterface[];
  }>(MY_FREINDS);
    ...
    <section className="section">
      ...
      <div className="columns is-centered">
        <div className="column has-text-centered is-one-quarter">
          <h1 className="has-text-weight-bold subtitle has-text-centered">
            Other users
          </h1>
          {all?.users
            ?.filter((u) => notInFriends(u, friends))
            .map((user: any) => (
                    ...
                    <nav className="level">
                      ...
                      <div className="level-right">
                        <div className="level-item">
                          <button
                            onClick={() =>
                              addFreind({ variables: { userId: user.id } })
                            }
                  ...
                  <nav className="level">
                    ...
                    <div className="level-right">
                      <div className="level-item">
                        <button
                          onClick={() =>
                            removeFriend({ variables: { userId: user.id } })
                          }
